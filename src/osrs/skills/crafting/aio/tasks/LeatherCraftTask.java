...
public class LeatherCraftTask extends Task {
    ...
    public LeatherCraftTask(AioCrafting bot) {
        this.bot = bot;
    }

    @Override
    public boolean validate() {
        return AioCrafting.itemType.equalsIgnoreCase("Leather");
    }
    ...
    private void bank() {
        if (Players.getLocal() != null && (!Inventory.contains(AioCrafting.hide) || !Inventory.contains("Needle") || !Inventory.contains("Thread"))) {
            if (!Bank.isOpen()) {
                if (Inventory.getSelectedItem() != null) {
