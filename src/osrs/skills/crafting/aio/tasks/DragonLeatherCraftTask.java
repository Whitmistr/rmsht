...
public class DragonLeatherCraftTask extends Task {
    ...
    public DragonLeatherCraftTask(AioCrafting bot) {
        this.bot = bot;
    }

    @Override
    public boolean validate() {
        return AioCrafting.itemType.equalsIgnoreCase("Dragon leather");
    }
    ...
    private void makeXHandler() {
        if (MakeAllInterface.isOpen()) {
            ...
            if (MakeAllInterface.selectItem(AioCrafting.itemName, true)) {
                ...
                Execution.delayWhile(() -> Inventory.getQuantity(AioCrafting.hide) > AioCrafting.hidesReq && !ChatDialog.isOpen(), () -> Objects.requireNonNull(local).getAnimationId() != -1, CustomPlayerSense.Key.JOJO_MIN_INTERACT_DELAY_LONG.getAsInteger(), CustomPlayerSense.Key.JOJO_MAX_INTERACT_DELAY_LONG.getAsInteger());
            }
        }
    }
    ...
    private void withdraw() {
        ...
        if (Inventory.contains("Needle") && Inventory.contains("Thread") && Inventory.getQuantity(AioCrafting.hide) < AioCrafting.hidesReq && !Inventory.containsAnyExcept("Needle", "Thread", AioCrafting.hide)) {
    ...
    private boolean shouldBank() {
        return needle == null || thread == null || hide == null || Inventory.getQuantity(AioCrafting.hide) < AioCrafting.hidesReq;
    }
