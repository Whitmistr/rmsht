...
public class Mode1Task extends Task {
    ...
    public Mode1Task(AioSmelting bot) {
        this.bot = bot;
    }

    public boolean validate() {
        return AioSmelting.mode.equals("1");
    }
    ...
    private void bank() {
        ...
        if (Inventory.containsAnyExcept(AioSmelting.item1)) {
            Bank.depositAllExcept(AioSmelting.item1);
            return;
        }
        withdraw();
    }
    ...
    private void reassignPath() {
        webPath = Traversal.getDefaultWeb().getPathBuilder().buildTo(walkToBank(AioSmelting.item1, AioSmelting.BANK_AREA) ? AioSmelting.BANK_AREA.getRandomCoordinate() : AioSmelting.FURNACE_AREA.getRandomCoordinate());
    }

    private boolean walkToBank(String thing, Area bankArea) {
        return !Inventory.contains(thing) && !bankArea.contains(local);
    }
}