...
public class Mode4Task extends Task {

    /*
     * This mode withdraws a mould and 27 items.
     */
    ...
    public Mode4Task(AioSmelting bot) {
        this.bot = bot;
    }

    public boolean validate() {
        return AioSmelting.mode.equals("4");
    }
    ...
    private void bank() {
        ...
        if (Inventory.getQuantity(AioSmelting.item1) > 27 || Inventory.getQuantity(AioSmelting.mould) > 1) {
            Bank.depositInventory();
            return;
        }
        if (Inventory.containsAnyExcept(AioSmelting.item1, AioSmelting.mould)) {
            Bank.depositAllExcept(AioSmelting.item1, AioSmelting.mould);
            return;
        }
        if (Inventory.isFull() && (!Inventory.contains(AioSmelting.item1) || !Inventory.contains(AioSmelting.mould))) {
            Bank.depositAllExcept(AioSmelting.item1, AioSmelting.mould);
            return;
        }
        withdraw();
    ...
    private void withdraw() {
        ...
        if (Inventory.contains(AioSmelting.mould) && Inventory.getQuantity(AioSmelting.item1) < AioSmelting.item1qt) {
            if (Bank.getQuantity(AioSmelting.item1) > AioSmelting.item2qt) {
                Execution.delay(CustomPlayerSense.Key.JOJO_MIN_IDLE_INTERACT_DELAY.getAsInteger(), CustomPlayerSense.Key.JOJO_MAX_IDLE_INTERACT_DELAY.getAsInteger());
    ...
    private void reassignPath() {
        webPath = Traversal.getDefaultWeb().getPathBuilder().buildTo(walkToBank(AioSmelting.item1, AioSmelting.mould, AioSmelting.BANK_AREA) ? AioSmelting.BANK_AREA.getRandomCoordinate() : AioSmelting.FURNACE_AREA.getRandomCoordinate());
    }
