...
public class Mode3Task extends Task {
    ...
    private SpriteItem primaryIngredient, secondaryIngredient, mortar;

    public Mode3Task(AioHerblore bot) {
        this.bot = bot;
    }

    public boolean validate() {
        return AioHerblore.mode.equals("3");
    }

    public void execute() {
        mortar = Inventory.getItems(AioHerblore.mortar).first();
    ...
    private void withdraw() {
        if (!Inventory.contains("Pestle and mortar")) {
            if (Bank.getQuantity("Pestle and mortar") > 0) {
                Execution.delay(CustomPlayerSense.Key.JOJO_MIN_IDLE_INTERACT_DELAY.getAsInteger(), CustomPlayerSense.Key.JOJO_MAX_IDLE_INTERACT_DELAY.getAsInteger());
                if (Bank.withdraw("Pestle and mortar", 1)) {
                    Execution.delayUntil(() -> Inventory.contains("Pestle and mortar"), CustomPlayerSense.Key.JOJO_MIN_INTERACT_DELAY.getAsInteger(), CustomPlayerSense.Key.JOJO_MAX_INTERACT_DELAY.getAsInteger());
                }
            } else {
                bot.stop("No pestle and mortar found.");
            }
            return;
        }
        if (Inventory.contains("Pestle and mortar") && Inventory.getQuantity(AioHerblore.primaryIngredient) < AioHerblore.primaryQt) {
        ...
        if (Inventory.contains("Pestle and mortar") && Inventory.getQuantity(AioHerblore.primaryIngredient) == AioHerblore.primaryQt && Inventory.getQuantity(AioHerblore.secondaryIngredient) < AioHerblore.seconrdaryQt) {
    ...
    private boolean shouldBank() {
        return primaryIngredient == null || secondaryIngredient == null || mortar == null;
    }
